# Guides

Task-focused recipes for setting up and operating Repowire.

## Connect agents

- [Connect Claude Code](connect-claude-code.md)
- [Connect Codex](connect-codex.md)
- [Connect Gemini CLI](connect-gemini.md)
- [Connect Antigravity CLI](connect-antigravity.md)
- [Connect OpenCode](connect-opencode.md)

## Use control surfaces

- [Use the dashboard](use-dashboard.md)
- [Use Telegram](use-telegram.md)
- [Use Slack](use-slack.md)
- [Send files and images](send-files.md)

## Run work

- [Spawn another peer](spawn-peer.md)
- [Schedule work](schedule-work.md)
- [Run jobs](run-jobs.md)
- [Run the remote dashboard](run-remote-dashboard.md)
- [Self-host the relay](self-host-relay.md)
