...
- **Session nudges** — when the selected session has a running agent attached, the dashboard can send fire-and-forget nudges such as status, checkpoint, and continue through `POST /sessions/{repowire_session_id}/controls/notify`. Sessions remain durable even when no agent is running; in that case the timeline can still show history, but the nudge controls are disabled until the daemon can resolve a running agent. Detached local-agent sessions with captured runtime ids should report resume availability rather than a permanent unsupported state.
